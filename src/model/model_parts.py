...
from .model_blocks import BottleneckBlock # TEMP


class ResNetBackbone(nn.Module):
    def __init__(self, in_channels, block, layers, rho=1, iters=20):
        super().__init__()
        self.rho = rho
        self.iters = 20
        self.in_ch = 64
    ...
    def _make_layers(self, block, out_ch, num_blocks, stride=1):
        if stride != 1 or self.in_ch != out_ch * block.expantion:
            ...
            downsample = None
        layers = list()
        layers.append(block(self.in_ch, out_ch, stride, downsample, self.rho, self.iters))
